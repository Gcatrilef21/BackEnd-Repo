    ...
    try {
        ...
        router.post('/',async (req,res)=>{
            ...
            if (!title || !description || !code || !price || !stock || !thumbnail) {
                return res.status(400).json({Error:'Estos son Campos Obligatorios : Titulo, Descripcion, Codigo, Precio, Stock, Thumbnail'});
            }

            let newProduct = await product.addProduct(title, description, code, price, stock,thumbnail)
        ...
        router.put('/:pid', (req,res)=>{
            ...
            return res.status(200).send('FUUUUAP, Acuestate');
        })
